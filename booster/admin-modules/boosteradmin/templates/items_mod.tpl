...
{foreach $datas_new as $data}
        <tr class="{cycle array('even','odd')}">
            ...
            <td><a href="{jurl 'boosteradmin~itemsreview:editnew',array('id'=>$data->id)}">{$data->name|eschtml}</a></td>
...
{foreach $datas_mod as $data}
    <tbody>
        <tr>
            <td><a href="{jurl 'boosteradmin~itemsreview:editmod',array('id'=>$data->id)}">{$data->name|eschtml}</a></td>
